...
"$project_root/training/keras_fix_input_shape.py" "$model_path/model.h5" "$model_path/model_fixedinputshape.h5"

# Convert from Keras to quantized TF Lite model
quantization="float_fallback_quant"
"$project_root/training/keras_to_tflite.py" "$model_path/model_fixedinputshape.h5" "$model_path/model.tflite" -o "$quantization" -d "$model_dataset"
