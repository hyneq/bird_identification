...
PredictionResultT = TypeVar(
    "PredictionResultT", bound=IPredictionResultWithBoundingBoxes
)
...
class FileExtractor(ABC, Generic[PredictionResultT]):
    ...
    def extract(self):
        ...
        for path in input_glob:
            name = "-".join(
                os.path.splitext(os.path.relpath(path, self.input_dir))[0].split(
                    os.path.sep
                )
            )
            ...
            for result in results:
                ...
                extracted_name = self.filename_template.format(
                    source_name=name, i=str(i).zfill(6)
                )
