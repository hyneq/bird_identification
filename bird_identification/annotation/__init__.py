...
class TextWithBackgroundAnnotation(IAnnotation):
    ...
    def __init__(
        ...
        (rect_width, rect_height), _ = cv2.getTextSize(
            text, DEFAULT_FONT, 1, DEFAULT_TEXT_THICKNESS
        )

        rect_origin = (origin[0], origin[1] - rect_height)

        self.background = RectangleAnnotation(
            BoundingBox(rect_origin[0], rect_origin[1], rect_width, rect_height),
            background_color,
            -1,
        )
...
class RectangleWithTextAnnotation(IAnnotation):
    ...
    def __init__(
        self, bounding_box: BoundingBox, text: str, color: Color, thickness: int = 3
    ):
