...
from .classes import (
    ...
    ClassSelector,
    ClassificationMode,
    ClassSelectorFactory,
    DEFAULT_CLASS_SELECTOR_FACTORY,
)
from .models import (
    IPredictionModel,
    ...
    PredictionModelWithClassesConfigT,
...
class IPredictionResultWithClassesAndBoundingBoxes(
    IPredictionResultWithClasses, IPredictionResultWithBoundingBoxes
):
    pass
...
class PredictionProcessorWithClasses(
...
PredictionProcessorWithClassesT = TypeVar(
...
class PredictionProcessorFactory(
    ABC, Generic[PredictionModelOutputT, PredictionResultT]
):
    prediction_processor_cls: type[
        PredictionProcessor[PredictionModelOutputT, PredictionResultT]
    ]

    def get_prediction_processor(
        self,
        model: IPredictionModel[Any, Any, PredictionModelOutputT],
        ...
        if not prediction_processor_cls:
            prediction_processor_cls = self.prediction_processor_cls

        return prediction_processor_cls(**p_kwargs)


@dataclass()
class PredictionProcessorWithClassesFactory(
    PredictionProcessorFactory[PredictionModelOutputT, PredictionResultT]
):
    prediction_processor_cls: type[
    ...
    def get_prediction_processor(
        self,
        model: IPredictionModelWithClasses[Any, Any, PredictionModelOutputT],
        prediction_processor_cls: Optional[
            type[
                PredictionProcessorWithClasses[PredictionModelOutputT, PredictionResultT]
            ]
        ...
        **kwargs
        ...
        model_class_names = model.get_class_names()
...
class APredictor(
    ABC, Generic[PredictionInputT_cls, PredictionModelInputT, PredictionResultT]
):
    __slots__: tuple
    ...
    def __init__(
        self, input_strategy: Optional[InputStrategy[Any, PredictionModelInputT]] = None
    ):
        if input_strategy:
    ...
    def predict(
        self,
        input: PredictionInputT_fun,
        input_strategy: InputStrategy[PredictionInputT_fun, PredictionModelInputT],
    ) -> PredictionResultT:
        pass

    def predict(
        self,
        input: Union[PredictionInputT_cls, PredictionInputT_fun],
        input_strategy: Optional[
            InputStrategy[PredictionInputT_fun, PredictionModelInputT]
        ] = None,
    ) -> PredictionResultT:
...
class Predictor(
    ...
    def __init__(
        ...
        prediction_processor: PredictionProcessor[
            PredictionModelOutputT, PredictionResultT
        ],
        input_strategy: InputStrategy[Any, PredictionModelInputT],
    ):
...
class PredictorWithClasses(
    ...
    prediction_processor: PredictionProcessorWithClasses[
        PredictionModelOutputT, PredictionResultT
    ]
...
class IPredictorFactory(
    ...
    def get_predictor(
        ...
        predictor: Optional[
            type[
                APredictor[
                    PredictionInputT_cls, PredictionModelInputT, PredictionResultT
                ]
            ]
        ] = None,
        input_strategy: Optional[
            InputStrategy[PredictionInputT_cls, PredictionModelInputT]
        ] = None,
        cs_config: Optional[ClassSelectorConfig] = None,
        cs: Optional[ClassSelector] = None,
        mode: Optional[ClassificationMode] = None,
...
class PredictorWithClassesFactory(
    IPredictorFactory[
        PredictorConfigT,
        PredictionModelConfigT,
        PredictionInputT_cls,
    ...
    prediction_processor_factory: PredictionProcessorWithClassesFactory[
        PredictionModelOutputT, PredictionResultT
    ]
    ...
    def get_predictor(
        ...
        prediction_processor: Optional[
            PredictionProcessorWithClasses[PredictionModelOutputT, PredictionResultT]
        ] = None,
        input_strategy: Optional[
            InputStrategy[PredictionInputT, PredictionModelInputT]
        ] = None,
        cs_config: Optional[ClassSelectorConfig] = None,
        cs: Optional[ClassSelector] = None,
        mode: Optional[ClassificationMode] = None,
        ...
        if not model:
            model = self.model_factory(
                factory=model_type, cfg_input=model_path, cfg=model_config
            )

        if not prediction_processor:
            prediction_processor = (
                self.prediction_processor_factory.get_prediction_processor(
                    model=model,
                    cs_config=cs_config,
                    cs=cs,
                    mode=mode,
